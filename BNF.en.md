...
Note: the semantic support state for typed declaration tags, derive traits, custom operators, and unsupported values is tracked in `docs/reference/semantic-capability-support-matrix.md`. `@[derive(Eq, Show)]` processes built-in derives in argument order; custom generators use `comptime meta.Type -> meta.Items` and attach with `@[expand(generator)]`. The compiler derives dependency order and fixed-point execution from typed protocol signatures rather than public scheduling clauses.
Note: `meta` is a compiler-built-in domain that needs no import. Typed reflection exposes read-only target-independent facts through `meta.Type`, `meta.Declaration`, `meta.Field`, and `meta.Function`; `meta.layoutOf(Type, target)` requires an explicit target. `meta.Items` accepts structured generated declarations, not string source insertion or arbitrary AST mutation.
...
Note: named `instance` declarations are semantically validated during naming. Member names/signatures must match the target trait, const-generic trait arguments are substituted into method signatures, and overlap is checked after alias canonicalization: strictly more specific heads may coexist with broader heads, while equivalent or incomparable canonical shapes are rejected with `E3004`. Function-level `@impl(...)` is removed from the 0.7 authoring surface and recognized only as migration input.
...
Note: ADT constructor lists use commas, for example `Direction :: type { North, South }`. Eidos 0.7 rejects `|` in an ADT body with a migration diagnostic; `eidosc migrate syntax --from 0.5.0-alpha.1 --to 0.6.0-alpha.1` performs the explicit rewrite.
