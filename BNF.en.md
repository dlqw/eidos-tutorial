...
Note: in `0.4.0-alpha.1` declaration/import contexts, module paths may use dot-separated module segments, for example `Demo.Main :: module`, `Parser :: import Compiler.Parser;`, and `import PackageAlias::Module.Path::{item}`. `::` remains the package and item qualification separator; migration rewrites only the module-path `/` separators in import declarations.
...
Note: syntax version `0.4.0-alpha.1` adds an initial name-first declaration surface. The implemented subset currently covers `Name :: module`, `name :: Type -> Type { ... }`, `name :: Type -> Type;`, `name :: comptime Type -> Type { ... }`, `name :: value;`, `name :: Type = value;`, `Name :: comptime expr;`, `Name :: type`, `Name :: trait`, `Name :: effect;`, `Name :: instance Trait[...] { ... }`, and `Alias :: import Module.Path;`. Legacy keyword-first forms remain documented here for the current default syntax while migration is still in progress.
...
Note: in `0.4.0-alpha.1`, `comptime name := expr;` marks a local compile-time binding, and `name :: comptime A -> B { ... }` declares a comptime-only function. `comptime mut` is not supported in the first implementation slice, assigning to a comptime binding is a semantic error, and runtime references or calls to comptime-only functions are rejected. The phase-1 comptime evaluator accepts literals, references to previously evaluated comptime bindings, grouped or curried calls to comptime-only functions with no named arguments or `with` clauses, tuples/lists without spread, simple pure unary/binary operators, `if` expressions whose condition is a comptime `Bool`, and `match` expressions over literal, integer range, wildcard, or-pattern, tuple, and list patterns. Comptime-only function bodies may call other comptime-only functions. By-value pattern, list rest, and pure pattern-guard bindings become comptime values in the selected branch, and selected branches may use pure `Bool` guards and result-only blocks. Ordinary runtime/static calls, blocks with statements, borrow bindings, effectful operations, named-argument comptime calls, partial comptime applications, and `with` clauses are still rejected as not yet comptime-evaluable. Scalar comptime references are inlined before MIR; comptime module bindings do not emit module value getters, and local comptime bindings do not create runtime locals.
...
Note: the semantic support state for attributes, derive traits, custom operators, and unsupported values is tracked in `docs/reference/semantic-capability-support-matrix.md`. `@derive(Eq, Show)` processes built-in derives in argument order.
Note: `@borrow(...)` on an effect declaration is accepted as metadata but does not grant borrow permissions. Borrow checking is independent from effect authorization.
Note: in the current 0.4.0-alpha.1 slice, `comptime T: Type` and `T: comptime Type` are accepted as type-level comptime generic parameters and are preserved through symbols and HIR. Value-level const generics such as `comptime N: Int` are parsed but rejected during naming until const-generic typechecking and lowering are implemented.
Note: `@impl(...)` is semantically validated during naming. The function name/signature must match the target trait method, generic trait arguments are not inferred by convention registration, and impl overlap is checked after alias canonicalization: strictly more specific heads may coexist with broader heads, while equivalent or incomparable canonical shapes are rejected with `E3004`.
...
Note: ADT constructor lists use commas in new code, for example `Direction :: type { North, South }`. Legacy `|` constructor separators remain parser-compatible during migration, but a single ADT constructor list must not mix `,` and `|`.
...
Note: `+:` is the sequence prepend operator (Scala-style) and `:+` is the sequence append operator. `head +: tail` lowers to `Std::Seq::cons(head)(tail)` (right-associative: `a +: b +: xs` = `a +: (b +: xs)`); `xs :+ last` lowers to `Std::Seq::append(xs)(Std::Seq::singleton(last))` (left-associative: `xs :+ a :+ b` = `(xs :+ a) :+ b`). Both operands share the cons precedence level. `::` is no longer an operator; it serves only static binding and path qualification.
