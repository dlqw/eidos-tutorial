...
说明：依赖包中的模块使用 `packageAlias.Module.Path`，其中 `packageAlias` 是当前项目 `eidos.toml` 中的依赖键。普通标准库必须显式声明依赖（惯例为 `std = "0.9.0-alpha.1"`），并写作 `std.Seq`、`std.Console` 等。编译器分发的 Prelude Core Image 不是 package，会自动 open，通常直接使用其中的裸符号；`[language].noImplicitPrelude = true` 可关闭自动 open，已移除的 `noImplicitStdlib` 会被拒绝。
...
说明：`+:` 是序列 prepend 运算符（Scala 风格），`:+` 是序列 append 运算符。`head +: tail` 降低为 `Std.Seq.cons(head)(tail)`（右结合：`a +: b +: xs` = `a +: (b +: xs)`）；`xs :+ last` 降低为 `Std.Seq.append(xs)(Std.Seq.singleton(last))`（左结合：`xs :+ a :+ b` = `(xs :+ a) :+ b`）。两者共享 cons 优先级。`::` 不是表达式运算符，只用于声明绑定。
